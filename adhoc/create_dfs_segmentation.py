...
for key_dir in [
    ...
    segmaps_full_paths = conn.list_objects(Bucket='zoomlion-sample',
                                           Prefix=f'{key_dir}/segmaps')['Contents']
    seg_label_paths.extend(
        [f'{key_dir}/segmaps/' + os.path.basename(key['Key']) for key in segmaps_full_paths])
...
df = pd.DataFrame(
    list(zip(img_paths, seg_label_paths, label_name, label, splits)),
