    ...
    if is_s3_path(path):
        if os.path.splitext(path)[1] != '':
            ...
            while True:
                ...
                response = s3.list_objects_v2(Bucket=bucket, Prefix=key, MaxKeys=1000, **tkn)
            ...
            paths = [
                os.path.join('s3://', bucket, s3_path['Key'])
...
def s3_file_to_local(s3_path, local_path=None, client=None):
    ...
    if not os.path.exists(path=local_path):
        raise IOError("File {} not found, download failed".format(local_path))
