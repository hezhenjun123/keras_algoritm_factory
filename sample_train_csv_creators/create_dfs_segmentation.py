...
def give_split(name, train_generated_only=False):
    if (train_generated_only or np.random.uniform() > (.5)) and 'generated' not in name:
        return 'valid'
    else:
        return 'train'
...
seg_label_paths = []
img_paths = []
for key_dir in ['wheat_dmg_segmentation', ]:
    segmaps_full_paths = conn.list_objects(Bucket='zoomlion-sample', Prefix=f'{key_dir}/segmaps')['Contents']
    seg_label_paths.extend([f'{key_dir}/segmaps/' + os.path.basename(key['Key']) for key in segmaps_full_paths])
    img_full_paths = conn.list_objects(Bucket='zoomlion-sample', Prefix=f'{key_dir}/imgs')['Contents']
    img_paths.extend([f'{key_dir}/imgs/' + os.path.basename(key['Key']) for key in img_full_paths])

# filter prism extras
img_paths = [x for x in img_paths if '_label.png' not in x and x[-1] != '/']
seg_label_paths = [x for x in seg_label_paths if '_label.png' not in x and x[-1] != '/']

assert len(img_paths) == len(seg_label_paths), 'Something is not matching in lengths'
